...
with graph.as_default():
  ...
  # Training computation.
  def logits(dataset):
  	return tf.matmul(tf.nn.relu(tf.matmul(dataset, weights_1) + biases_1), weights_2) + biases_2
  #hypoth_1 = tf.nn.relu(tf.matmul(tf_train_dataset, weights_1) + biases_1)
  #logits = tf.matmul(hypoth_1, weights_2) + biases_2
  #logits = tf.matmul(tf_train_dataset, weights) + biases
  loss = tf.reduce_mean(
    tf.nn.softmax_cross_entropy_with_logits(logits(tf_train_dataset), tf_train_labels))
